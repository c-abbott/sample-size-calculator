...
const ExpandableContent: React.FC<{ title: string; content: string }> = ({
  title,
  content,
}) => {
    ...
    >
      ...
      >
        <FontAwesomeIcon
          ...
          }`} // Conditional class for opacity
        />
        {title}
      </button>
      {isOpen && (
        <p className="mt-2 ml-8 px-4 py-4 text-sm text-gray-400">{content}</p>
      )}
    </div>
  );
};
